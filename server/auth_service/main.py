...
async def register(user_data: schemas.UserCreate, db: Session = Depends(get_db)):
    ...
    send_email(
        to=user_data.email,
        subject="Herbal Garden - Account Created",
        html=f"""
        <h3>Welcome to Herbal Garden 🌿</h3>
        <p>Your username: <strong>{user_data.email}</strong></p>
...
async def forgot_password(data: schemas.ResetCodeCreate, db: Session = Depends(get_db)):
    ...
    send_email(
        ...
        <p>Reset code: <strong>{reset_id}</strong></p>
